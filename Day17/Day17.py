...
class ConwayCubeCalculator:
    def __init__(self, input):
        ...
        for (y,line) in enumerate(input):
            for (x,item) in enumerate(line):
                if '#' == item:
                    self.__activateCube((x,y,0))
        self.life = 0

    def __updateLimits(self, pos):
        if self.limits is None:
            self.limits = [(pos[0],pos[0]),(pos[1],pos[1]),(pos[2],pos[2])]
        else:
            for i in range(3):
        ...
        for x in range(limits[0][0]-1, limits[0][1]+2):
            for y in range(limits[1][0]-1, limits[1][1]+2):
                for z in range(limits[2][0]-1, limits[2][1]+2):
                    if self.__determineActiveFromState((x,y,z),state):
                        self.__activateCube((x,y,z))
        self.life += 1
    ...
    def __getNeighbors(self, pos):
        for z in [-1,1]:
            for x in [-1,0,1]:
                for y in [-1,0,1]:
                    yield (x+pos[0],y+pos[1],z+pos[2])

        for x in [-1,1]:
            for y in [-1,0,1]:
                yield (x+pos[0],y+pos[1],pos[2])
        
        for y in [-1,1]:
            yield (pos[0],y+pos[1],pos[2])
