...
def add_edge(graph, from_type, to_type, cost):
    ...
    graph['edges'][(from_type,to_type)] = cost
...
def find_container_types_holding(graph, type):
    starters = []
    for edge in graph['edges']:
        if edge[1] == type:
            starters.append(edge[0])
    return starters

graph = parse_graph(readInput())
#print(graph)

types = find_container_types_holding(graph, 'shiny gold')
starters = set()
for type in types:
    if type not in starters:
        starters.add(type)
        this_starters = find_container_types_holding(graph, type)
        types += this_starters
